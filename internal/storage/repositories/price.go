...
func (repo *PriceRepository) SaveFirstSymbolPrices(ctx context.Context, prices []domain.SymbolPrice) error {
	...
	query := fmt.Sprintf(
		"INSERT INTO crypto_loader.first_symbol_price(price, symbol,exchange,datetime) VALUES %s ON CONFLICT (symbol, exchange) DO NOTHING",
		strings.Join(values, ", "),
	)
	_, err := repo.db.ExecContext(ctx, query)
	if err != nil {
...
func (repo *PriceRepository) FirstSymbolPrices(ctx context.Context) ([]domain.SymbolPrice, error) {
	var (
		query = `
SELECT price, symbol, exchange, datetime, updated_at
FROM first_symbol_price
`
...
func (repo *PriceRepository) FirstSymbolPrice(ctx context.Context, exchange, symbol string) (domain.SymbolPrice, error) {
	var (
		query = `
SELECT price, symbol, exchange, datetime, updated_at
FROM first_symbol_price 
...
LIMIT 1
`
		price domain.SymbolPrice
	)
	if err := repo.db.GetContext(ctx, &price, query); err != nil {
